 ...
 */
import { React, Navigation, NavigationNative, NavigationStack, StyleSheet } from 'enmity/metro/common';
 ...
 */
export default ({ name = 'Page', component = View, CustomNavigationStack = NavigationStack.createStackNavigator() } = {}) => {
  /**
  ...
  const Button = ({onPress, title}) => {
    return (
        <TouchableOpacity onPress={onPress}>
            <Text style={[styles.text, { marginLeft: 16 }]}>{title}</Text>
        </TouchableOpacity>
    );
  };
  ...
  return <NavigationNative.NavigationContainer independent={true}>
    <CustomNavigationStack.Navigator
